...
- растров в градусных CRS: у них единицы сетки не метры, поддержка
  отложена;
Сплиты, наоборот, **готовы** — колонка `split` в манифесте. Делятся не по
площадкам, а по географическим кластерам: соседние вылеты снимают смежную
территорию, и площадка из train могла оказаться в трёхстах метрах от площадки
в held-out. Подробности и проверки — `DATASET_OAM_V2_REPORT.md`.
