...
**4053 обучающих пар** с **612 площадок**, 8.85 ГБ.
...
|---|---|---|---|
| `base` — Орто ↔ спутниковая подложка | 3022 | 421 | боевой тип: сторона A — вид виртуального борта, отрендеренный из ортоплана, сторона B — кроп спутниковой подложки Esri. Учит переносу между источниками |
| `ss` — Ортоплан сам на себя (same_source) | 744 | 590 | обе стороны из одного растра, разметка точна по построению. Учит инвариантности к ракурсу, наклону и сезонной перекраске |
...
Виды пар: `orto_basemap` — 3216, `same_source` — 811, `cross_date` — 26.

Высота съёмки 175–300 м (медиана 230), наклон камеры 0–20° (медиана 4.2°).
...
|---|---|---|---|---|
| `train` | 2393 | 2027 | 359 | обучение |
| `val` | 620 | 440 | 103 | выбор чекпоинта |
| `heldout` | 1040 | 749 | 150 | приёмка, расходуется один раз |
...
|---|---|---|
| `same_source` | **0.025 px** | фазовой корреляцией: стороны различаются только геометрией, измеритель точен |
