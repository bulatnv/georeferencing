...
META_HELP = {
    ...
    # поля пар, собранных из внешнего OrthoLoC (scripts/convert_ortholoc.py)
    "source": "происхождение пары: ortholoc — внешний датасет, orto_basemap — свой конвейер",
    "split": "сплит исходного датасета, из которого взят сэмпл",
    "sample": "идентификатор сэмпла OrthoLoC, из которого построена пара",
    "rectified": "ректифицирован ли кадр на сетку ортофото (режим rect)",
    "b_window": "окно стороны B в пикселях исходного ортофото: x, y, ширина, высота",
}


def mval(meta: dict, key: str, default="—"):
    """Значение поля меты или прочерк.

    Формат пары общий для корпусов, а набор полей — нет: у пар OrthoLoC нет
    курса и следа на земле, у наших нет режима ректификации. Отсутствующее
    поле — прочерк в тексте, а не падение разбора.
    """
    v = meta.get(key)
...
высоты {mval(meta, 'height_m')} м под наклоном {mval(meta, 'tilt_deg')}° и курсом
{mval(meta, 'yaw_deg')}°. Сторона B — квадратный кроп подложки {b.shape[1]}×{b.shape[0]},
повёрнутый на свой курс {mval(meta, 'yaw_b_deg')}°; разница курсов —
{mval(meta, 'delta_yaw_deg')}°, и именно её матчер должен преодолеть.</p>""")
    parts.append(fig(a, f"<b>image_a_jpeg</b> — сторона A, {a.shape[1]}×{a.shape[0]} px, "
                     f"GSD {mval(meta, 'gsd_a')} м/пкс, след на земле "
                     f"{mval(meta, 'footprint_a_m')} м"))
    parts.append(fig(b, f"<b>image_b_jpeg</b> — сторона B, {b.shape[1]}×{b.shape[0]} px, "
                     f"GSD {mval(meta, 'gsd_b')} м/пкс, охват "
                     f"{mval(meta, 'footprint_b_m')} м. "
...
это и есть <code>covis_frac</code> = {mval(meta, 'covis_frac')} из метаданных.
