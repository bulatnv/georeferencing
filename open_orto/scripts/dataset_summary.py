...
def readme(root: Path, rows, control) -> str:
    ...
    lines += [
        ...
        "площадки либо неподтверждённая площадка. Контрольная ось",
        "`same_source` сокращена физически (3218 пар → 811, покрытие всех",
        "612 площадок сохранено), а вес держит её долю в смеси на 12 %.",
        "",
        ...
        "## Документы рядом",
        "",
        "- `TRAINING.md` — **как обучать на этом датасете**: загрузчик, точки",
        "  подмены GT в тренерах, сэмплер по весам, что мерить и чего не делать;",
        ...
        "  манифеста, перемешивание, вывод случайной пары со всеми массивами и",
        "  проверками. То же, что в `PAIR_ANATOMY.html`, но на любой паре;",
