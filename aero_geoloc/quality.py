...
def assess(
    ...
    *,
    ...
    min_inliers_hard: int = 6,
    min_ncc: float = 0.3,
    inlier_saturation: int = 30,
) -> QualityAssessment:
    """Свести геометрию решения и сигналы в ковариацию, доверие и статус.

    Статус опирается на **калиброванную** неопределённость: решение считается
    ``LOCALIZED``, если 1σ-эллипс достаточно мал, инлайеров не критически мало и
    (если дан) фотометрический NCC не провален. Иначе — ``LOW_CONFIDENCE``:
    привязка есть, но доверять ей нельзя. Малое число инлайеров само раздувает
    эллипс, поэтому порог по эллипсу уже частично ловит слабые решения; жёсткий
    порог инлайеров — лишь защита от вырожденных конфигураций.
    ...
    Args:
        ...
        min_inliers_hard: жёсткий минимум инлайеров.
        min_ncc: минимальный NCC (если задан) для ``LOCALIZED``.
