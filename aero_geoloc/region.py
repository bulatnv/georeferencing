...
def estimated_build_seconds(plan: RegionPlan, *, seconds_per_cell: float = 0.005) -> float:
    """Грубая оценка времени сборки, чтобы предупредить ДО её начала.

    Коэффициент замерен на этом окружении (GPU, батч 8, FP16, тайлы уже в кэше):
    4232 клетки собрались за 19 с. **Загрузка тайлов сюда не входит** и на новом
    районе как раз доминирует — поэтому наружу оценка идёт с оговоркой.

    Точность здесь не важна; важно, чтобы владелец до начала знал порядок:
    секунды это или минуты.
    """
