...
__all__ = ["LocateRequest", "build_request", "InputError", "recommended_sigma_m",
           "PRIOR_ENVELOPE"]

#: Регламент «высота съёмки → допустимая σ приора» (веха 2026-07-27 в JOURNAL.md).
#:
#: Отпечаток кадра ∝ высоте, клетка карты ≈ отпечатку, поэтому σ обязана расти
#: вместе с высотой — иначе число клеток в районе выходит из надёжной зоны
#: (~200–500) и верное место тонет среди похожих. Регламент выводился аналитически
#: и подтверждён замером на `DRZ_00755` (90 м AGL): σ = 2 км даёт 10 000 клеток и
#: отказ, σ = 0.5 км — 1.7 м.
#:
#: **Шире — не «надёжнее на всякий случай», а прямо хуже.** Это ровно та ошибка,
#: на которой владелец потерял первый запуск.
PRIOR_ENVELOPE: tuple[tuple[float, float], ...] = (
    (100.0, 500.0),
    (300.0, 2000.0),
    (600.0, 5000.0),
)
...
def recommended_sigma_m(altitude_m: float) -> float:
    """Рекомендованная σ приора [м] для высоты съёмки — по :data:`PRIOR_ENVELOPE`.

    Между узлами — линейная интерполяция, снаружи — крайние значения. Это
    рекомендация, а не ограничение: приор дотягивается и дальше, но ценой веера
    кандидатов, на котором Этаж 1 перестаёт различать место.
    """
    nodes = PRIOR_ENVELOPE
    if altitude_m <= nodes[0][0]:
        return nodes[0][1]
    if altitude_m >= nodes[-1][0]:
        return nodes[-1][1]
    for (h0, s0), (h1, s1) in zip(nodes, nodes[1:]):
        if h0 <= altitude_m <= h1:
            return s0 + (s1 - s0) * (altitude_m - h0) / (h1 - h0)
    return nodes[-1][1]                      # недостижимо, но пусть будет явно
